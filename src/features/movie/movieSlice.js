...
let count = 0;
let { get_trending, get_latest, get_now_playing, get_popular, get_top_rate, get_up_coming, movie_genre, genres, get_video_id, get_trailer, get_image_original, get_image, get_logo, get_logo_image, } = TMDB_API
...
export function fetchMovieGenre(genreid, page) {
    const urlGenre = movie_genre(genreid, "", page)
    return async (dispatch) => {
...
export const { setLoading, setItems, setTopBannerVideo, setTopBannerData, setTopBannerVideo_play, add_func_child, setTopBannerVideo_url, setMoviePage_currGenre, setMoviePage_currPage,
    setMoviePage_loading, setMoviePage_loadNextPage, setMoviePage_data } = movieSlice.actions
